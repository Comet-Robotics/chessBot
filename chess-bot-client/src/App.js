...
function App() {
  ...
  const onSquareClick = async (piece) => {
    if (Object.keys(moveSquares).includes(piece)) {
      const res = await fetch(`/move/${selectedSquare}/${piece}`, {method:"POST"})
      setSelectedSquare("")
      setMoveSquares({})
  ...
  return (
    <div className="App">
        <div className="flex">
          ...
          <Chessboard
            ...
            position={Object.entries(query.data.pieces).reduce((acc, curr)=>{
              ...
              acc[position.toLowerCase()] = `${color}${piece.toUpperCase()}`
              return acc
            }, {})}
        ...
        <div className="enarc">
          ...
          <img className="enarcImage" src="https://nesscampbell.com/wp-content/uploads/kiL8jgmMSafPY6mv15PCVtAJzUpv0QGE1604495671-3.jpg" />
        </div>
    </div>
  );
}
